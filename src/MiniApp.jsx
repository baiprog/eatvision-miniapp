...
// --- Экстрактор названия блюда — только одно-два слова первой строки ---
function extractDishTitle(gptText) {
  if (!gptText) return '';
  // 1. Если есть "Блюдо:", берем только это, дальше до точки/запятой/новой строки/двоеточия/скобки
  const dishMatch = gptText.match(/Блюдо:\s*([^\n,.():;]+)/i);
  if (dishMatch && dishMatch[1]) {
    // Берём только первые два слова
    return dishMatch[1].trim().split(' ').slice(0,2).join(' ').replace(/^[-—–]+/, '').slice(0, 30);
  }
  // 2. Вырезаем "На фотографии ...", берём первые два слова первой строки
  let firstLine = gptText.replace(/^На фотографии.*?:?\s*/i, '').split('\n')[0];
  let words = firstLine.trim().split(/[,.!;:()]/)[0].split(' ').filter(Boolean).slice(0, 2);
  if (words.length) return words.join(' ').slice(0, 30);
...
export default function MiniApp() {
  ...
  const handleFileChange = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        // Строгий промпт для GPT
        const PROMPT = `
Всегда отвечай только так:
[название блюда, только одно-два слова]

Далее дай краткое описание, состав, калории, белки, жиры и углеводы.
НЕ используй слова типа "Блюдо:", "На фотографии", "Изображено", просто название блюда первой строкой!
Пример:
Паста карбонара
Итальянское блюдо из спагетти с соусом. Калории: 510, Белки: 20, Жиры: 12, Углеводы: 78
        `;
